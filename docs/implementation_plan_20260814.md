...
> 2026-08-28 状态：Studio S1.1–S1.5 已发布 `main`；S2.0、S2.1–S2.4 与 P1–P5 已完成编码、Codex 独立验收和艾卡人工体验确认。最终完整套件为 `1226 passed, 2 warnings, 0 skipped`。当前进入文档与 GitHub 发布收尾，下一开发阶段为 **S3 YOLOv8 Classify 产品闭环与 YOLOv5 Detect 遗留适配**。
...
4. ✅ **已完成并验收（2026-08-21，Studio S1.2，PR #4 已合并）**：数据集划分改为完整物化、确定性、可校验和原子发布的不可变快照，不再移动原始文件；普通训练和非 dry-run 自动调优绑定已校验快照。
5. ✅ **已完成并验收（2026-08-21，Studio S1.3，PR #4 已合并）**：API Key 迁移到 Windows Credential Manager/环境变量，提供客户设置、连接测试、保守迁移、自定义 Endpoint 策略、统一脱敏和轮换流程；旧 Key 已轮换并清除本机明文。
6. ✅ **已完成并验收（2026-08-24，Studio S1.4，PR #5 已合并）**：正式目录入口统一执行路径、允许根、权限、链接/reparse point、成员数与容量预检；遗留数据集 ZIP 和训练 ZIP/JSON 上传 API 固定返回 `410 LEGACY_UPLOAD_DISABLED`，不读取请求文件体且不继续建设 ZIP 安全提取。
7. **已完成（Studio S1.5，统一批次，2026-08-26 验收）**：普通训练与自动调优已统一运行身份和状态 Schema；完成 PID 创建身份校验、阶段与最后事件原子持久化、后台控制器与 SSE 解耦、有界事件补发，以及 API/SSE/UI 六态刷新重连。旧状态保持可读；PID 复用不误判；服务重启后保守标记中断或未知，不伪造恢复。
8. **已完成独立验收（Studio S2.0，4–7 人日）**：SQLite Schema v1/迁移/备份/故障保护、数据集与实验索引、产物引用、旧 JSON 幂等导入、基础 API/UI 和 JSON 回退均已完成；SQLite 不替代事实文件，也不复制数据集、权重或训练产物。
9. ✅ **已完成验收（Studio S2.1–S2.4）**：对账/重建/启动补录、详情/数据集关联/产物可用性、搜索/排序/分页/数据库诊断和 2–5 实验基础比较已完成，并通过人工试用。
10. **S2 明确排除**：不开发标签、备注、收藏、永久管理标识、归档、批量删除、自动移动/清理产物、Excel 导出、AI 自动评选最佳实验；不把不同数据集快照或缺失指标伪装成严格可比。
11. **Studio S3 训练任务与框架产品闭环（建议 10–18 人日）**：YOLOv8 Detect 回归冻结（2–4 人日）、YOLOv8 Classify 产品闭环（6–10 人日）和 YOLOv5 Detect 遗留适配（2–4 人日）。三者完成后冻结任务、命令、指标、报告、历史/SQLite 和 UI 支持矩阵；Segment、OBB、Pose、新版 YOLO 后置。
12. **Studio S4 Linux适配与产品封装（建议 10–20 人日）**：在 S3 完成后实施 Ubuntu Server、Docker Compose/NVIDIA GPU、原生安装/systemd、只读 Secret 文件、联网/离线安装、升级回退和最终试装；三条正式训练链路均须在目标环境完成真实短训练。

**顺序结论**：Studio 当前研发主线调整为 **S2.0（已验收）→ S2.1–S2.4统一扩展 → S3训练任务与框架闭环 → S4 Linux适配与产品封装**。Linux 测试机、Docker/NVIDIA 运行时、离线依赖、最小分类/YOLOv5 数据和 Secret 文件方案可以并行准备，但安装包冻结、正式部署验收和发布必须等待 S3 支持矩阵冻结。

### 2026-08-27 产品体验修复批次

1. ✅ **P1：调参建议、自动调优和终局TXT稳定保存**。真实三轮调优完成，最佳目录生成 `tuning_final_summary.txt`；审计、终态、SQLite和 `best.pt` 存在性完成核验。AI文字内容可信度不在本批完成定义内。
2. ✅ **P2：参考训练与数据集快照绑定**。历史参考运行绑定自身快照；禁止全局 `latest_dataset` 静默覆盖，命令、审计、历史和 SQLite 身份一致。
3. ✅ **P3：智能分析最近训练快捷选择**。训练路径输入框下展示 SQLite 最近 4 个安全可用训练结果，点击只回填目录，不自动触发分析。
4. ✅ **P4：数据库字段中英文动态翻译**。字段标签、来源、状态、阶段、产物和结构化枚举跟随语言切换；内部稳定枚举和数据库事实值不改写。
5. ✅ **P5：报告与审计表格查看**。详情中提供独立“查看报告/查看审计结果”弹窗；事实指标、参数、数据集关联、逐轮决策、产物完整性和终局状态可追溯。不生成 PDF，不在切换语言时调用 LLM。
6. **后续专项：AI输出可信度**。修复早停事实矛盾、混淆矩阵语义、最佳/最终轮次口径、负向指标描述、模型上下文和机器校验；该专项不与P2–P5混批。

发布门：P1–P5 已通过验收，艾卡于 2026-08-28 明确批准整理文档并推送 GitHub。发布只纳入源码、测试、脱敏模板和当前研发文档，排除数据集、图片、标签、模型权重、训练结果、日志、SQLite 运行库、审计和真实配置。
...
**S1.2 独立验收证据（2026-08-21）**：S1.2 定向套件 **110 passed**；当时完整套件 **285 passed, 2 warnings, 0 skipped**，两条 warning 仍为既有 sklearn PCA 数值警告。受控最小 Detect 数据验证源目录成员、大小、mtime 和 SHA-256 前后不变，相同请求复用、损坏快照拒绝；从项目工作目录使用快照绝对 `data.yaml` 完成真实 1 epoch YOLOv8 Detect；Chromium 验证快照区块、无快照状态、创建表单和非法参数反馈。S1.2 与 S1.3 已通过 PR #4 合并到 `main`。

**S1.3 独立验收证据（2026-08-21）**：S1.3 九文件套件 **180 passed**；完整套件 **396 passed, 2 warnings, 0 skipped**。模拟供应商验证禁止重定向及错误正文/假密钥不泄漏；随机 Windows Credential Manager 测试 target 完成跨进程读取并清理；Chromium 验证客户设置、密码框不预填、旧凭据提示和内网 HTTP 警告；艾卡使用轮换后的真实 Key 完成保存、连接和重新大模型分析。旧 YAML 字段及 5 个历史明文文件已清理。S1.2 与 S1.3 已通过 PR #4 合并到 `main`。

**S1.4 独立验收证据（2026-08-24）**：定向套件 **176 passed**；完整套件 **460 passed, 2 warnings, 0 skipped**。Chromium 验证不存在/越界路径、成员超限、容量超限、目录浏览错误、按钮恢复、中文原因与稳定错误码，无新增控制台错误。额外审查发现两个分析入口未展示 `error_code`，已按 TDD 补测试并修复。本批未触及训练执行与指标解析，因此未追加短 epoch 训练。

**S1.5 独立验收证据（2026-08-26）**：最终边界定向测试 **28 passed**；S1.5 六文件聚合 **84 passed**；完整套件 **546 passed, 2 warnings, 0 skipped**。真实断线调优验证确认客户端断开后后台控制器继续执行并持久化真实终态，浏览器刷新显示一致状态；Retention 上限为 20 个已完成控制器、TTL 30 分钟，单运行事件缓冲 2000 条。当前数据集快照损坏，因此未绕过安全门禁追加短 epoch，进入后续真实训练前必须先重建合法快照。

...
> 2026-08-26 状态：A0 审计闭环和统一训练收尾已进入 `v0.1`；Studio S1.1 已进入 `v0.2`；Studio S1.2 与 S1.3 已通过 PR #4 合并 `main`；Studio S1.4 已通过 PR #5 合并 `main`；Studio S1.5 运行状态与重连可靠性已完成独立验收，待本批提交。下一开发批次为 Studio S2.1 SQLite 本地存储基础。
...
8. **后续跨平台小批次（S1.3.1，不属于本次已实现范围）**：为 Linux/Docker 增加只读 Secret文件来源，验证普通文件、绝对路径、权限、大小、符号链接拒绝、缓存失效和容器重启轮换；Linux Secret Service随正式Linux桌面适配实施。

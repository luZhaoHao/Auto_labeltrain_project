...
> 2026-08-26 状态：A0 审计闭环和统一训练收尾已进入 `v0.1`；Studio S1.1 已进入 `v0.2`；Studio S1.2 与 S1.3 已通过 PR #4 合并 `main`；Studio S1.4 已通过 PR #5 合并 `main`；Studio S1.5 运行状态与重连可靠性已完成独立验收并推送 `codex/studio-s1.5`，待合并 `main`。下一开发批次为 Studio S2.1 SQLite 本地存储基础。
...
8. **下一批（Studio S2.1，建议 2–3 人日）**：建立 SQLite 单用户本地存储基础，包括 Schema 版本、连接与事务边界、数据库初始化、备份/损坏保护、JSON 只读迁移入口和回滚策略；本批不同时展开多数据集 UI 与历史对比页面。
9. **S2 后续小批次（合计 6–12 人日）**：S2.1 完成后依次实施 S2.2 数据集索引（2–3 人日）、S2.3 实验/报告/产物索引和旧 JSON 幂等迁移（1–3 人日）、S2.4 数据集切换、历史对比、标签、归档与 UI（1–3 人日）。
10. **S2.5 三任务闭环（10–18 人日）**：在 S2 稳定后，依次完成 YOLOv8 Detect 回归（2–4 人日）、YOLOv8 Classify 产品闭环（6–10 人日）和 YOLOv5 Detect 遗留适配（2–4 人日）；三项均须通过真实短训练、指标、历史、报告、失败路径和 UI 验收。
11. **S3 Linux与安装交付（10–20 人日）**：在 S2.5 完成后冻结 SQLite、配置、持久化目录和任务依赖，再实施 Ubuntu Server 兼容、Docker Compose 首选交付、原生安装/systemd、联网/离线安装、升级回退和试装验证。
12. **后续跨平台小批次（S1.3.1，随 S3 前段实施）**：为 Linux/Docker 增加只读 Secret文件来源，验证普通文件、绝对路径、权限、大小、符号链接拒绝、缓存失效和容器重启轮换；Linux Secret Service随正式Linux桌面适配实施。

**顺序结论**：Studio 当前研发主线固定为 **S2 → S2.5 → S3**，初步总量约 **26–50 人日**。Linux 测试机、Docker/NVIDIA 运行时、离线依赖、最小分类/YOLOv5 数据和 Secret 文件方案可以与前序开发并行准备，但安装包冻结、正式部署验收和发布必须等待 S2 与 S2.5 完成。

...
> 2026-08-24 状态：A0 审计闭环和统一训练收尾已进入 `v0.1`；Studio S1.1 已进入 `v0.2`；Studio S1.2 与 S1.3 已通过 PR #4 合并 `main`；Studio S1.4 目录输入安全已通过 PR #5 合并 `main`。下一开发批次为 Studio S1.5 运行状态与重连可靠性，验收后再进入 S2 SQLite 与多数据集。
...
7. **下一批（Studio S1.5，统一批次，3–6 人日）**：统一普通训练与自动调优的运行身份和状态 Schema；同时完成 PID 身份校验、阶段与最后事件原子持久化，以及 API、SSE、UI 刷新/重连。旧状态保持可读；PID 复用不得误判；异常消失标记为中断或未知，不伪造恢复。本批内部可按 Schema、进程校验、重连展示三个任务实施，但统一设计、开发、验收和提交。

...
> 2026-08-24 状态：A0 审计闭环和统一训练收尾已进入 `v0.1`；Studio S1.1 已进入 `v0.2`；Studio S1.2 与 S1.3 已通过 PR #4 合并 `main`；Studio S1.4 目录输入安全已完成独立验收，等待文档闭环、提交和推送。下一开发批次转入 Studio S2 SQLite 与多数据集规格。
...
6. ✅ **已完成并验收（2026-08-24，Studio S1.4，待推送）**：正式目录入口统一执行路径、允许根、权限、链接/reparse point、成员数与容量预检；遗留数据集 ZIP 和训练 ZIP/JSON 上传 API 固定返回 `410 LEGACY_UPLOAD_DISABLED`，不读取请求文件体且不继续建设 ZIP 安全提取。

...
当前稳定版本为 `v0.1`，A0 审计闭环与统一训练收尾已经完成独立验收并发布。下次开发不应重新实现或重构这两部分；应从本文件列出的剩余事项中选择一个小批次，先形成规格和测试边界，再交由 Claude Code 实现业务代码，最后由 Codex 独立复验和发布。
...
1. A0.2 剩余可靠性事项：训练日志默认仅显示 epoch 关键指标，完整日志折叠查看。
2. 数据集划分改为不可变快照，不移动原始文件。
3. API 密钥迁移到环境变量，并制定已暴露密钥轮换步骤。
4. 补充目录选择、上传总量、成员数量和解压后容量限制。
5. 完成上述可靠性事项后，再进入 A1 的 YOLO11/YOLO26 与 task/version 配置框架。

...
当前已发布稳定版本仍为 `v0.2`；A0 审计闭环、统一训练收尾以及 Studio S1.1 已发布。Studio S1.2 与 S1.3 已通过 PR #4 合并到 `main`（合并提交 `e0092dee`，未新建 Release）。Studio S1.4 目录输入安全已于 2026-08-24 完成 Codex 独立验收，等待本次文档闭环、提交和推送。下次开发不得重新实现或破坏这些兼容边界。
...
### 3.6 Studio S1.4 目录输入安全（2026-08-24 已验收，待推送）
...
1. 完成 Studio S1.4 文档闭环、敏感信息复扫、提交和推送；不新建 Release，除非艾卡另行要求。

...
当前已发布稳定版本仍为 `v0.2`；A0 审计闭环、统一训练收尾以及 Studio S1.1 已发布。Studio S1.2 与 S1.3 已通过 PR #4 合并到 `main`（合并提交 `e0092dee`）；Studio S1.4 目录输入安全已通过 PR #5 合并到 `main`（合并提交 `47daf33f`）。上述合并均未新建 Release。下次开发不得重新实现或破坏这些兼容边界。
...
### 3.7 Studio S1.5 运行状态与重连可靠性（2026-08-26 已验收，待提交）
...
最近结果（2026-08-26，S1.5 验收）：`546 passed, 2 warnings, 0 skipped`。
...
1. Studio S1.5 已完成代码、独立验收并推送 `codex/studio-s1.5`，待合并 `main`；后续保持统一状态、PID 身份、后台控制器、短期事件重放和保守中断语义。
2. 下一批进入 Studio S2.1：先建立 SQLite 本地存储基础、Schema 版本、事务边界、备份/损坏保护和 JSON 只读迁移入口，不同时展开 UI 大改。
3. S2.1 稳定后依次进入 S2.2 数据集索引、S2.3 实验索引与迁移、S2.4 UI 对比与归档。
4. S2 合计预计 6–12 人日；稳定后进入 S2.5 三任务闭环（10–18 人日），只产品化验收 YOLOv8 Detect、YOLOv8 Classify 和 YOLOv5 Detect。YOLO11/YOLO26 当前暂缓，不作为近期完成标准。
5. S2.5 完成后再进入 S3 Linux与安装交付（10–20 人日）。当前主线 **S2 → S2.5 → S3** 合计约 26–50 人日；Linux/Docker环境、离线依赖、测试数据和 Secret 文件方案可以提前准备，但不得提前冻结安装包或跳过前序验收。
